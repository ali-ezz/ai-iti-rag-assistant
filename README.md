...
Full results are in [`data/evaluation/results.csv`](data/evaluation/results.csv), regenerated by CI (`.github/workflows/pipeline.yml`) every time the corpus or notebook changes: **264 chunks · 12 documents · 12 test questions · Ollama `qwen2.5:1.5b`**.

| Metric | Result |
|---|---|
| Retrieval hit rate @4 | **12/12 (100%)** — expected lab appears in top-4 chunks |
| Answers with citations | **12/12 (100%)** — every answer contains `[Source N]` markers |
| Grounded / hallucination check | Manual review of `results.csv`: factual claims supported by retrieved lab sections; refusal path when context is insufficient |

**Sample row** (question → retrieved source → verdict):

| Question | Top source | Retrieval correct | Citation grounded |
|---|---|---|---|
| How does Non-Max Suppression remove duplicate detections? | LAB-05-Object-Detection | ✓ | ✓ |
| What is the difference between stemming and lemmatization? | LAB-08-NLP-Preprocessing | ✓ | ✓ |
| Why do Transformers require positional encoding? | LAB-12-Transformers | ✓ | ✓ |
| Which preprocessing steps were used for Arabic text? | LAB-11-Arabic-NLP | ✓ | ✓ |

**Failure analysis & mitigations** (notebook section 7): overlapping terminology across labs (e.g. “edge detection” spans LAB-03/04/05), questions broader than one section, and a small CPU LLM producing terse or occasionally truncated answers. Mitigations: section-aware chunks, 60-word overlap, top-4 retrieval, explicit source metadata, low-temperature (`0.1`) generation, a strict context-only system prompt, and an insufficient-context refusal. Remaining misses are addressed by clearer document headings or a reranker — never by allowing unsupported model knowledge.

CI run: [pipeline · main](https://github.com/ali-ezz/ai-iti-rag-assistant/actions/workflows/pipeline.yml).
